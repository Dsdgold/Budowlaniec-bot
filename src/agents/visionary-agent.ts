...
export class VisionaryAgent extends BaseAgent {
  ...
  private async brainstorm(context: string): Promise<number> {
    const systemPrompt = [
      'Jestes startup founderem. Platforma Spektra: monitoring cen materialow budowlanych w Polsce.',
      'Stack: TypeScript, Express, PostgreSQL. Strona klienta: public/client.html.',
      '',
      'Wygeneruj DOKLADNIE 2 pomysly jako JSON array. Kazdy pomysl to ui_change (sekcja HTML na strone).',
      'Kod HTML musi byc KOMPLETNY — gotowy do wstawienia w strone. Max 50 linii per pomysl.',
      'Sekcja moze zawierac <style> i <script> i pobierac dane z API.',
      '',
      'Dostepne API: /api/stats, /api/content/posts, /api/webapp/prices/cement, /api/pricing, /api/webapp/news',
      '',
      'POMYSLY na sekcje: FAQ, testimoniale, "jak to dziala", porownanie cen, trending produkty,',
      'kalkulator budowy, formularz kontaktowy, statystyki rynku, ostatnie alerty cenowe.',
      '',
      'Format: [{"title":"Nazwa sekcji","description":"co robi","priority":"high","type":"ui_change","code":"<section>HTML</section>","target_file":"public/client.html"}]',
      'Odpowiedz TYLKO JSON array. Bez tekstu. Zaczynaj od [.',
    ].join('\n');
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: 'Kontekst:\n' + context.substring(0, 500) + '\n\nWygeneruj 2 sekcje HTML na strone. TYLKO JSON array. Zaczynaj od [.',
      }],
    });
